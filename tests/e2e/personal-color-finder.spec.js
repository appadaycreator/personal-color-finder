...
const SVC_URL = "https://appadaycreator.github.io/personal-color-finder/";
const SVC_NAME = "パーソナルカラー診断";

test.describe("パーソナルカラー診断 - E2Eテスト", () => {

  test("ページ基本: タイトル・h1・canonical", async ({ page }) => {
    await page.goto(SVC_URL);
    const title = await page.title();
    expect(title.length).toBeGreaterThan(15);
    expect(title.toLowerCase()).toMatch(/パーソナルカラー/i);
    const canonical = await page.locator('link[rel="canonical"]').getAttribute("href");
    expect(canonical).toContain("personal-color-finder");
  ...
  test("SEO: sitemap.xml・robots.txt が存在する", async ({ page }) => {
    const sitemapResp = await page.request.get("https://appadaycreator.github.io/personal-color-finder/sitemap.xml");
    expect([200, 301, 302]).toContain(sitemapResp.status());
    const robotsResp = await page.request.get("https://appadaycreator.github.io/personal-color-finder/robots.txt");
  ...
  test("プライバシー・利用規約: ページが存在する", async ({ page }) => {
    const privResp = await page.request.get("https://appadaycreator.github.io/personal-color-finder/privacy-policy.html");
    const termsResp = await page.request.get("https://appadaycreator.github.io/personal-color-finder/terms.html");
